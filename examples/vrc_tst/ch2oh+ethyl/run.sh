...
if [ $irun -eq 1 ] || [ $irun -eq 0 ]; then
    ...
    sed -ie "s/ntauo_ts/Tauo_ts/g" data/estoktp.dat
...
if [ $irun -eq 2 ] || [ $irun -eq 0 ]; then
	...
	if [[ $npoint -eq $numdirs ]]; then
	    ...
	    sed -ie "s/guess=(mix,always)//g" data/theory.dat
...
if [ $irun -eq 25 ] || [ $irun -eq 0 ]; then
    for npoint in $(seq "$numfull" "$numdirs" )
    do
	nprog=$(($npoint+1))
	var="dist$nprog"
	name=dist_"${!var}"
	cp -R dist_$dist6 $name
	cp -f ref/data/estoktp.dat ./$name/data
	cp -f ref/data/ts.dat ./$name/data
	cd ./$name
	sed -ie "s/STEPN/Step2/g" data/estoktp.dat
	sed -ie "s/DISTN/${!var}/g" data/ts.dat
    sed -ie "s/nang dih/ang dih/g" data/ts.dat
	sed -ie "s/nOpt_ts_1/Opt_ts_1/g" data/estoktp.dat
	sed -ie "s/n1dTau_ts/1dTau_ts/g" data/estoktp.dat
	sed -ie "s/nHL_ts/HL_ts/g" data/estoktp.dat
	sed -ie "s/nSymm_ts/Symm_ts/g" data/estoktp.dat
	...
	if [[ $npoint -eq $numdirs ]]; then
	    ...
	    sed -ie "s/guess=(mix,always)//g" data/theory.dat
	    sed -ie "s/{frequencies;print,hessian}//g" output/ts_asl1_step2.inp
        sed -ie "s/ang dih/nang dih/g" data/ts.dat
	fi
...
# now check if step25 calculations finished
...
# now proceed to step 45 to prepare VRC-TST required data for correction potential

if [ $irun -eq 45 ] || [ $irun -eq 0 ]; then
...
# now check if step 45 calculations finished
...
# to activate to run rotd when active
...
# step 7 re-run for new potential 

if [ $irun -eq 7 ] ; then

    name=variational
    cp -f ref/data/vrc_as.inp ./$name/data
    cp -f ref/data/theory.dat ./$name/data
    cp -f ref/data/estoktp.dat ./$name/data
    cp -f ref/data/ts.dat ./$name/data
...
#    cp -R dist_$dist1 $name
	FILE2=./$name/failed
	if [ ! -f "$FILE2" ]; then
# cp ref/data/vrc_as.inp variational/data/
	    cp -f ref/data/vrc_as.inp ./$name/data
	    cp -f ref/data/theory.dat ./$name/data
	    cp -f ref/data/estoktp.dat ./$name/data
	    cp -f ref/data/ts.dat ./$name/data
...
#    sed -ie "s/nHL_ts/HL_ts/g" data/estoktp.dat
#    sed -ie "s/nSymm_ts/Symm_ts/g" data/estoktp.dat
	    sed -ie "s/npot_corr/pot_corr hl/g" data/estoktp.dat
	    if [[ $npoint -eq $numdist ]]; then
        	sed -ie "s/pot_corr/pot_corr inf/g" data/estoktp.dat
    	    fi	
	    rm -f ./finished
