...
c         stop
      ...
      read(15,*)icharge2,ispin2,iguess2
      close(15)
...
C     *****************************
      ...
      dimension q(nmdmx)
      dimension freq(nmdmx),tau(ntaumx),tauopt(ntaumx)
      dimension coord(natommx,ndim),xint(3*natommx),xint_save(3*natommx)
     $ ,abcrot(ndim) 
      dimension ibond(nmdmx)
...
cc         call readgrad_molpro(np,atomlabel,grad)
         call readgrad_molpro(nint,atomlabel,grad)
cc         do j=1,np
         do j=1,nint
            if(ibond(j).eq.1)then
cc               grad(j)=grad(j)
               grad(j)=grad(j)*CAUTOANG
            else
      ...
      enddo
      if(iguess2.eq.2)then
         write(7,*)'**coord   value   gradient **'
         do j=1,nint
            write(7,100)atomlabel(j),q(j),grad(j)
         enddo
         write(7,*)
      endif
